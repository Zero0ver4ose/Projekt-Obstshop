...
function warenkorbErstellen() {
    let anzMango:number = parseFloat((document.getElementById("auswahlMango") as HTMLInputElement).value)
    ...
    containerBezahlen?.replaceChildren() 
        ...
        for (let k in auswahl) {
            if (bestand5[i].name == auswahl[k].name) {
                ...
                if (auswahl[k].anzahl <= obstanzahl) {
                    ...
                    containerBezahlen?.appendChild(fehlerMeldung)
                }
